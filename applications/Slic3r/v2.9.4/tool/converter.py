...
def parse_cpp_options(content: str) -> tuple[List[Dict[str, Any]], Dict[str, Any]]:
    ...
    for idx, m in enumerate(matches):
        ...
        enum_call = re.search(
            r'def->set_enum<[^>]+>\s*\(\s*\{([^}]*)\}\s*\)', block, re.S)
        if enum_call:
        ...
        d_str = re.search(
            r'set_default_value\(new\s+ConfigOptionString\("([^"]*)"\)\)', block)
        if d_str:
            default = d_str.group(1)

        d_float = re.search(
            r'set_default_value\(new\s+ConfigOptionFloat\(\s*([0-9eE\+\-\.]+)\s*\)\)', block)
        if d_float:
            default = d_float.group(1)

        d_int = re.search(
            r'set_default_value\(new\s+ConfigOptionInt\(\s*([0-9eE\+\-\.]+)\s*\)\)', block)
        if d_int:
            default = d_int.group(1)

        d_bool = re.search(
            r'set_default_value\(new\s+ConfigOptionBool\(\s*(true|false)\s*\)\)', block)
        if d_bool:
            default = d_bool.group(1)

        d_enum = re.search(
            r'set_default_value\(new\s+ConfigOptionEnum<[^>]+>\(([^)]+)\)\)', block)
        if d_enum:
            ...
            if options:
                ...
                for o in options.keys():
                    if o.lower() == def_tok.lower() or o.replace('-', '').lower() == def_tok.lower():
                        match_opt = o
                        break
                default = match_opt if match_opt is not None else (
                    list(options.keys())[0] if options else tok)
            else:
                default = tok

        # Fallback: sometimes code uses set_default_value(new ConfigOptionString());
        d_empty_str = re.search(
            r'set_default_value\(new\s+ConfigOptionString\(\)\)', block)
...
def apply_parent_mapping(categories: List[Dict[str, Any]], params: List[Dict[str, Any]]) -> None:
    ...
    for child, parent in PARENT_MAP.items():
        ...
        if child not in name_to_cat:
            ...
            new_child["color"] = RAINBOW_COLORS[len(
                categories) % len(RAINBOW_COLORS)]
        ...
        if parent not in name_to_cat:
            ...
            # assign next color
            new_cat["color"] = RAINBOW_COLORS[len(
                categories) % len(RAINBOW_COLORS)]
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Convert Slic3r PrintConfig.cpp to LamiNode schema")
    parser.add_argument("output", nargs="?", help="Optional output path")
    parser.add_argument(
        "-o", "--output", dest="opt_output", help="Output path")
    ...
    output_path = Path(args.opt_output or args.output) if (
        args.opt_output or args.output) else default_output
    ...
    for version in ["v0.1", "v0.1a"]:
        ...
        out_file.write_text(json.dumps(
            schema, indent=4, ensure_ascii=False), encoding="utf-8")
